-- Agrega SC_ID_MATCH como nuevo tipo de match en la vista de comparación.
-- Los nuevos campos bigin_*_scid se agregan al final para no romper dependencias.
...
select 'Sellercloud customers' as metric, count(*)::text as value
from sellercloud_customers

union all

select 'Bigin active contacts', count(*)::text
from bigin_contacts

union all

select 'Email and name match', count(*)::text
from customer_bigin_comparison_v2
where match_status = 'EMAIL_AND_NAME_MATCH'

union all

select 'Email match, name different', count(*)::text
from customer_bigin_comparison_v2
where match_status = 'EMAIL_MATCH_NAME_DIFFERENT'

union all

select 'Name match, email different', count(*)::text
from customer_bigin_comparison_v2
where match_status = 'NAME_MATCH_EMAIL_DIFFERENT'

union all

select 'SC ID match', count(*)::text
from customer_bigin_comparison_v2
where match_status = 'SC_ID_MATCH'

union all

select 'Pending review', count(*)::text
